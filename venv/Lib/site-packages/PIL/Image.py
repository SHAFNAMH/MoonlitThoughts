...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def close(self) -> None:
        ...
        # object is gone.
        self._im = DeferredError(ValueError("Operation on closed images"))
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"images mode={self.mode} size={self.size[0]}x{self.size[1]} "
            f"at 0x{id(self):X}>"
        )
    ...
    def _repr_image(self, image_format: str, **kwargs: Any) -> bytes | None:
        ...
        :returns: images as bytes, saved into the given format.
        """
        b = io.BytesIO()
        try:
    ...
    def _repr_jpeg_(self) -> bytes | None:
        ...
        :returns: JPEG version of the images as bytes
        """
    ...
    def __getstate__(self) -> list[Any]:
        im_data = self.tobytes()  # load images first
    ...
    def tobytes(self, encoder_name: str = "raw", *args: Any) -> bytes:
        ...
        .. warning::
            ...
            This method returns raw images data derived from Pillow's internal
    ...
    def tobitmap(self, name: str = "images") -> bytes:
        """
    ...
    def frombytes(
        ...
        *args: Any,
        ...
        if s[0] >= 0:
            msg = "not enough images data"
            raise ValueError(msg)
        if s[1] != 0:
            msg = "cannot decode images data"
            raise ValueError(msg)
    ...
    def convert(
        ...
        When translating a color images to grayscale (mode "L"),
        ...
        channel from the images will be used instead of the values from the palette.
    ...
    def quantize(
        ...
        if palette:
            ...
            palette.load()
            if palette.mode != "P":
                msg = "bad mode for palette images"
                raise ValueError(msg)
    ...
    def getbbox(self, *, alpha_only: bool = True) -> tuple[int, int, int, int] | None:
        ...
        :param alpha_only: Optional flag, defaulting to ``True``.
           If ``True`` and the images has an alpha channel, trim transparent pixels.
    ...
    def getcolors(
        ...
        The colors will be in the images's mode. For example, an RGB images will
        return a tuple of (red, green, blue) color values, and a P images will
    ...
    def getexif(self) -> Exif:
        """
        Gets EXIF data from the images.
    ...
    def has_transparency_data(self) -> bool:
        """
        Determine if an images has transparency data, whether in the form of an
        alpha channel, a palette with an alpha channel, or a "transparency" key
        in the info dictionary.

        Note the images might still appear solid, if all of the values shown
        within are opaque.
    ...
    def apply_transparency(self) -> None:
        """
        If a P mode images has a "transparency" key in the info dictionary,
        remove the key and instead apply the transparency to the palette.
        Otherwise, the images is unchanged.
        """
    ...
    def histogram(
        ...
        """
        Returns a histogram for the images. The histogram is returned as a
        list of pixel counts, one for each pixel value in the source
        images. Counts are grouped into 256 bins for each band, even if
        the images has more than 8 bits per band. If the images has more
        than one band, the histograms for all bands are concatenated (for
        example, the histogram for an "RGB" images contains 768 values).

        A bilevel images (mode "1") is treated as a grayscale ("L") images
        by this method.
        ...
        bi-level images (mode "1") or a grayscale images ("L").
    ...
    def entropy(
        ...
        A bilevel images (mode "1") is treated as a grayscale ("L")
        ...
        either a bi-level images (mode "1") or a grayscale images ("L").
    ...
    def paste(
        ...
        :param im: Source images or pixel value (integer, float or tuple).
    ...
    def alpha_composite(
        ...
        # over images, crop if it's not the whole images.
    ...
    def point(
        ...
        :param mode: Output mode (default is same as input). This can only be used if
           the source images has mode "L" or "P", and the output has mode "1" or the
    ...
    def putalpha(self, alpha: Image | int) -> None:
        ...
        :param alpha: The new alpha layer.  This can either be an "L" or "1"
           images having the same size as this images, or an integer.
        """
    ...
    def putdata(
        ...
        """
        Copies pixel data from a flattened sequence object into the images. The
        values should start at the upper left corner (0, 0), continue to the
        end of the line, followed directly by the first value of the second
        line, and so on. Data will be read until either the images or the
    ...
    def putpalette(
        ...
        """
        Attaches a palette to this images.  The images must be a "P", "PA", "L"
        or "LA" images.
    ...
    def putpixel(
        ...
        * :py:mod:`~PIL.ImageDraw`
        ...
        if (
            ...
            # RGB or RGBA value for a P or PA images
            if self.mode == "PA":
    ...
    def resize(
        ...
        :param resample: An optional resampling filter.  This can be
           ...
           If the images has mode "1" or "P", it is always set to
    ...
    def rotate(
        ...
        :param resample: An optional resampling filter.  This can be
           ...
           interpolation in a 4x4 environment). If omitted, or if the images has
    ...
    def save(
        ...
        :param params: Extra parameters to the images writer. These can also be
           set on the images itself through ``encoderinfo``. This is useful when
           saving multiple images::

             # Saving XMP data to a single images
             from PIL import Image
             red = Image.new("RGB", (1, 1), "#f00")
             red.save("out.mpo", xmp=b"test")

             # Saving XMP data to the second frame of an images
             from PIL import Image
    ...
    def show(self, title: str | None = None) -> None:
        ...
        On Unix, the images is then opened using the **xdg-open**, **display**,
    ...
    def getchannel(self, channel: int | str) -> Image:
        ...
        if isinstance(channel, str):
            try:
                ...
                msg = f'The images has no channel "{channel}"'
                raise ValueError(msg) from e
...
def new(
    ...
    :param color: What color to use for the images. Default is black. If given,
       ...
       color is None, the images is not initialised.
    :returns: An :py:class:`~PIL.Image.Image` object.
    """
...
def fromarray(obj: SupportsArrayInterface, mode: str | None = None) -> Image:
    ...
    :param mode: Optional mode to use when reading ``obj``. Since pixel values do not
      ...
      grayscale L mode data within a P mode images, or read RGB data as YCbCr for
      example.
...
def fromarrow(
    ...
    """Creates an images with zero-copy shared memory from an object exporting
...
def open(
    ...
    msg = "cannot identify images file %r" % (filename if filename else fp)
...
def alpha_composite(im1: Image, im2: Image) -> Image:
    ...
    :param im1: The first images. Must have mode RGBA or LA.
    :param im2: The second images. Must have the same mode and size as the first images.
    :returns: An :py:class:`~PIL.Image.Image` object.
    """
...
def register_mime(id: str, mimetype: str) -> None:
    """
    Registers an images MIME type by populating ``Image.MIME``. This function
    should not be used in application code.

    ``Image.MIME`` provides a mapping from images format identifiers to mime
...
class Exif(_ExifBase):
    """
    This class provides read and write access to EXIF images data::
